...
def first_mate_charge_system(team: str, system: str) -> Dict:
    ...
    if system not in SUBMARINE_SYSTEMS:
    ...
    logging.info(f"System '{system}' for Team '{team}' is now at charge {sub.gauges[system]}/{SUBMARINE_SYSTEMS[system]}.")

    # Check if the system is now fully charged
    if sub.gauges[system] >= SUBMARINE_SYSTEMS[system]:
        sub.systems[system] = True
        sub.gauges[system] = SUBMARINE_SYSTEMS[system] # Cap at max charge
