...
PATH_TO_YOUR_MCP_SERVER_SCRIPT_TIC = "/home/odoo/src/break-fast-google-hackathon-2025/tic_tac_toe/server.py" # <<< REPLACE

mcp_submarine = MCPToolset(
    connection_params=StdioConnectionParams(
        server_params=StdioServerParameters(
            command='python3',  # Command to run your MCP server script
            args=[PATH_TO_YOUR_MCP_SERVER_SCRIPT_TIC],  # Argument is the path to the script
        )
    )
...
def create_player_agent(player_marker: str, opponent_marker: str) -> LlmAgent:
    ...
    return LlmAgent(
        ...
        instruction=(
            ...
            "Print the board as a grid after each move. Nicely formated"
        ),
